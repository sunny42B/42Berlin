/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   functions_two_bonus.c                              :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: sung-hle <sung-hle@student.42berlin.de>    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/05/24 16:50:07 by sung-hle          #+#    #+#             */
/*   Updated: 2023/11/23 16:24:01 by sung-hle         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "so_long_bonus.h"

int	return_free_split(char **map, char *s)
{
	ft_free(map);
	ft_error_sl(s);
	return (1);
}

int	line_check(t_vars *vars, int i)
{
	int		j;
	size_t	k;

	j = -1;
	while (++j < i)
	{
		k = -1;
		if (j == 0 || j == i - 1)
			while (++k < ft_strlen(vars->map[j]))
				if (vars->map[j][k] != '1')
					return (0);
		k = -1;
		while (++k < ft_strlen(vars->map[j]))
			if (k == 0 || k == (ft_strlen(vars->map[j]) - 1))
				if (vars->map[j][k] != '1')
					return (0);
	}
	return (1);
}

int	same_length(t_vars *vars, int i)
{
	int		j;
	size_t	len;

	j = 0;
	len = ft_strlen(vars->map[j]);
	while (++j < i)
		if (ft_strlen(vars->map[j]) != len)
			return (0);
	return (1);
}

char	**dup_string_arr(char **original, int length)
{
	int		i;
	char	**new;
	int		len;

	i = 0;
	new = malloc((length + 1) * sizeof(char *));
	if (new == NULL)
		return (NULL);
	while (i < length)
	{
		len = ft_strlen(original[i]) + 1;
		new[i] = malloc(len * sizeof(char));
		if (new[i] == NULL)
		{
			ft_free(new);
			return (NULL);
		}
		ft_strlcpy(new[i], original[i], len);
		i++;
	}
	new[i] = NULL;
	return (new);
}

int	which_instance(t_vars *vars)
{
	int	i;

	i = 0;
	while (i < (int)vars->coll->count)
	{
		if (vars->coll->instances[i].x == vars->blink->b->instances[0].x && \
			vars->coll->instances[i].y == vars->blink->b->instances[0].y)
			return (i);
		i++;
	}
	return (-1);
}
